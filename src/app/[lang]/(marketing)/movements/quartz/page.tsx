...
export async function generateMetadata({ params }: { params: Promise<{ lang: Locale }> }): Promise<Metadata> {
  ...
  return {
    title: dict.meta.movements.quartz.title,
    description: dict.meta.movements.quartz.description,
    openGraph: { title: dict.meta.movements.quartz.title, description: dict.meta.movements.quartz.description },
  };
}
...
export default async function QuartzPage({ params }: { params: Promise<{ lang: Locale }> }) {
  const { lang = 'es' } = await params;
  const dict = await getDictionary(lang);
  const content = dict.movements.quartz;
      ...
      <section className="py-16">
        <div className="container-golden max-w-4xl">
          ...
          <div className="space-y-4">
            {content.famousExamples.models.map((w, i) => (
              <div key={i} className="bg-white dark:bg-card p-6 rounded-xl shadow-luxury">
